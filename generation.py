...
============================================
Generates loop patrol paths up to MAX_PATH_LEN cells long and ranks them
by avg_path_danger: the mean danger of the cells the drone actually flies
through. This directly rewards paths that stay in the hottest zones,
regardless of path length.

Movement is 8-directional (cardinal + diagonal) and loop closing also
allows 8-directional adjacency so the two are consistent.

Outputs
-------
  drone_paths_raw.csv       — all generated loop paths + metrics
  drone_paths_filtered.csv  — top N by avg_path_danger, deduplicated
...
def generate_loop_paths(in_park: np.ndarray, max_len: int = MAX_PATH_LEN):
    ...
    Yield all simple loop paths of length 3..max_len that:
      ...
      - use 8-directional moves
      - end on a cell 8-directionally adjacent to the start (closing the loop)
    ...
    while stack:
        ...
        # Try closing the loop — last cell must be 8-adjacent to first
        if len(path) >= 3:
            sr, sc = path[0]
            cr, cc = current
            dr = abs(cr - sr)
...
def path_length_km(path, rows_km, cols_km) -> float:
    """Total flight distance including closing leg back to start."""
    pts   = path + [path[0]]
    total = 0.0
...
def compute_metrics(path, danger, in_park, rows_km, cols_km) -> dict:
    ...
    # avg_path_danger: mean danger of cells the drone literally flies through
    # This is the primary ranking metric — rewards flying through hot zones
    ...
    # Also compute danger_removed for reference (used by optimizer later)
...
def canonical(path_str: str) -> str:
    """Same loop traversed in reverse gets the same key."""
...
def main():
    ...
    print("=" * 60)
    print("  Drone Loop Path Generator — Etosha")
    print("=" * 60)
    ...
    # Generate
    print(f"\n[2/4] Generating loop paths (max {args.max_len} cells, cardinal) ...")
    t0 = time.time()
    ...
    # Filter — rank by avg_path_danger
    ...
    lines.append(f"Movement          : 8-directional")
    lines.append(f"Ranking metric    : avg_path_danger (mean danger of flown cells)")
