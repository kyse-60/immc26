...
# ── Config ────────────────────────────────────────────────────────────────────
CSV_PATH        = "newanimal_maps.csv"
SHP_PATH        = "your_park_boundary.shp"   # ← update to your .shp path
...
for i, name in enumerate(animal_names):
    z   = prob_matrix[i]      # sighting probabilities at the 16 lodges
    w   = prices[i] * populations[i]   # value weight for this animal
    ...
    # Zero out outside the park before accumulating
    prob_grid_masked = np.where(inside_mask, prob_grid, np.nan)
    value_grid       = prob_grid_masked * w

    animal_maps[name] = {"prob": prob_grid_masked, "value": value_grid}

    # Accumulate total (nansum so NaN outside park stays NaN)
    if i == 0:
    ...
    print(f"  {name:<22s}  P range [{z.min():.2f}, {z.max():.2f}]  "
          f"weight={w:>12,.0f}")

print(f"\nTotal value range: [{np.nanmin(total_value):.0f}, {np.nanmax(total_value):.0f}]")
...
# ── 8. Optional: export per-animal probability maps ──────────────────────────
# Uncomment if you want to inspect individual species maps:
#
# for name, maps in animal_maps.items():
#     fig2, ax2 = plt.subplots(figsize=(10, 6), facecolor="#0d1117")
#     ax2.set_facecolor("#0d1117")
#     ax2.pcolormesh(XI, YI, maps["prob"], cmap="YlGn",
#                   norm=mcolors.Normalize(0, 1), shading="auto")
#     ax2.set_title(name, color="white")
#     plt.tight_layout()
#     plt.savefig(f"map_{name.replace(' ','_')}.png", dpi=120,
